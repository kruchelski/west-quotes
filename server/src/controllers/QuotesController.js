...
const generateQuote = async (req, res) => {
  try {
    ...
    // Split the quote
    const splitQuoted = quoteRaw.split(' ');

    // Generates random indexes based on the quote length
    const index1 = Math.floor(Math.random() * splitQuoted.length);
    const index2 = Math.floor(Math.random() * splitQuoted.length);

    // Extract quote fragment for image search
    const imageQuery = `${splitQuoted[index1]} ${splitQuoted[index2]}`;
...
const likeQuote = async (req, res) => {
  ...
  try {
    ...
    const quote = await Quote.findByPk(quoteUuid, { transaction });
    ...
    const [userQuote, created] = await UserQuote.findOrCreate(
      {
        where: {
          uuid_user: req.user.uuid,
...
/**
 * Remove a quote, identified by the uuid in the params, from the favorites
 * @param {*} req Request object
 * @param {*} res Response object
 */
const removeQuote = async (req, res) => {
  ...
  try {
    ...
    if (!quoteUuid) {
      throw new QuoteError('Error trying to remove quote from favorites', 400, "Quote's UUID");
    }
...
module.exports = {
  generateQuote, index, getQuoteDetails, likeQuote, removeQuote,
};
